...
namespace RoseOnlineBot
{
    public static class CustomMarshal
    {
        ...
        public static void StructureToPtr(object structure, IntPtr ptr, byte[] data, bool fDeleteOld)
        {
            ...
            long extraDataOffset = 0;
            long structBase = (long)ptr.ToInt64();
            long structSize = (long)Marshal.SizeOf(structure);

            foreach (FieldInfo field in structure.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance))
            {
                long fieldLoc = structBase + (long)Marshal.OffsetOf(structure.GetType(), field.Name);
                if (field.IsDefined(typeof(CustomMarshalAsAttribute), true))
                {
                    ...
                    for (int i = 0; i < bytes.Length; i++, extraDataOffset++)
                    {
                        if (i < data.Length) 
                            ...
                            bytes[i] = 0x00; // overwrite unicode formatting shit 
