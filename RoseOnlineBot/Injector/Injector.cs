...
namespace RoseOnlineBot
{
    public class Injector : IDisposable
    {
        ...
        public void EjectLibrary(string libName, ProcessModule mod = null)
        {
            string libSearchName = File.Exists(libName) ? Path.GetFileName(Path.GetFullPath(libName)) : libName;
            
            if (!injectedModules.ContainsKey(libSearchName) && mod == null)            
                throw new InvalidOperationException("That module has not been injected into the process and thus cannot be ejected");
            else if (mod != null && !injectedModules.ContainsKey(libSearchName))
                injectedModules.Add(libSearchName, new InjectedModule(mod));
        ...
        /// <returns><see cref="IntPtr"/> representing the return value of function <paramref name="funcName"/> in module <paramref name="libName"/></returns>
        public IntPtr CallExport<T>(string libName, string funcName, T data, byte[] _data) where T : struct
        {
            return CallExport<T>((uint)ThreadWaitValue.Infinite, libName, funcName, data, _data);
        }
        ...
        /// <param name="data">Data of type <typeparamref name="T"/> to be sent as argument to remote function</param>
        /// <returns><see cref="IntPtr"/> representing the return value of function <paramref name="funcName"/> in module <paramref name="libName"/></returns>
        public IntPtr CallExport<T>(uint timeout, string libName, string funcName, T data, byte[] _data) where T : struct
        {
            ...
            try
            {
                ...
                CustomMarshal.StructureToPtr(data, pData, _data, true);
        ...
        private IntPtr CallExportInternal(uint timeout, string libName, string funcName, IntPtr data, Type dataType, uint dataSize)
        {
            ...
            try
            {
                ...
                long singleObject = Imports.WaitForSingleObject(hThread, timeout);
